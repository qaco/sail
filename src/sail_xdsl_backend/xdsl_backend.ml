...
let process_nexp_aux (nexp_aux: nexp_aux): mlir_type = match nexp_aux with
    Nexp_id(id) -> NotImplemented("Nexp_id")
  | Nexp_var(Kid_aux(Var(v),_)) -> NotImplemented("Nexp_var")
  | Nexp_constant(num) ->
     let mlir_type = match Nat_big_num.to_int(num) with
         2 -> Builtin(Scalar(I2))
       | 4 -> Builtin(Scalar(I4))
       | 8 -> Builtin(Scalar(I8))
       | 16 -> Builtin(Scalar(I16))
       | 32 -> Builtin(Scalar(I32))
       | 64 -> Builtin(Scalar(I64))
       | n -> Builtin(MultiDim(Tensor([n],I2))) in
     mlir_type
...
let rec process_dec_spec typs_env defs_env (Typ_aux(reg,_)): mlir_type = match reg with
    Typ_id id -> Typ_id (string_of_id id)
  | Typ_var(kid) -> NotImplemented("Typ_var")
  | Typ_app(id,typ_arg_list) ->
     let mlir_types_list = List.map (process_typ_arg typs_env defs_env) typ_arg_list in
...
and process_typ_arg typs_env defs_env (A_aux(typ_arg_aux, _)) = match typ_arg_aux with
    A_nexp(Nexp_aux(nexp_aux,_)) -> (* numeric expression, of kind Int *)
     process_nexp_aux nexp_aux
  | A_typ(t) -> process_dec_spec typs_env defs_env t
...
let process_typ_def_aux typs_env defs_env typ_def_aux = match typ_def_aux with
    TD_abbrev(id,typquant,typ_arg) ->
     ...
     TypeDef(name,process_typ_arg typs_env defs_env typ_arg)
...
let process_def_aux typs_env defs_env (DEF_aux(aux,_)) = match aux with
  ...
  | DEF_register(DEC_aux(DEC_reg(reg,id,_),_)) -> (* register declaration *)
     let name = string_of_id id in
     RegDef(name, process_dec_spec typs_env defs_env reg)
